...
For today's lecture we will be going through version control and at the end we will talk a bit more about `make`. In particular we are going to talk about how you can automate an `R` project with `make`. 

The lecture will be quite hands on and will help you get started with `Git`. Please note that this is a very basic introduction and does not constitute a deep dive into `Git`. I will link some resources below to further explore the magic of `Git`.
...
In addition, you might be working with projects that don't focus primarily on `R`. 

I only use a handful of shell commands in my daily workflow, so I won't burden you with too many. 

# `Git` shell commands

The easiest command is to clone a repo. I use this a lot. 
