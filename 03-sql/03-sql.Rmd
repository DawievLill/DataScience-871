...
We could also access the full dataset for South Africa by downloading it from the website in CSV format. The CSV file is in the `data` directory for this lecture, so you don't have to download it. It is called `wid-data-za.csv`. In this section we will show you how to utilise the command line to convert the CSV to an SQLite database. 
